...
Interested in both directions of traffic
...
#### i. calculating the total routes per traffic counter
...
## outliers
mapview(at_traffic_count_2018_proj[at_traffic_count_2018_proj$total_drivers_within_20m > 5000, c(2:17, 25:27)])

mapview(at_traffic_count_2018_proj[ at_traffic_count_2018_proj$Direction == "Both", c(2:17, 25:27)])

ifelse(at_traffic_count_2018_proj$Direction == "Both", )

at_traffic_count_2018_proj$total_drivers_within_20m
...
mapview(cas_data_akl_geo[1:12000, ], alpha = 0.1,  alpha.regions = 0.1, cex = 2)
...
mapview(jtw_driving_akl_SA2_cas_dep2[c("SA2_name_usual_residence_address", "total_cas_SA2_source", "total_driving_travellers", "total_traveler_times_cas", "cas_per_SA2_res", "SA2_average_NZDep2018", "SA2_average_NZDep2018_score")], 
        zcol = "cas_per_SA2_res", 
        alpha = 0.1,
        layer.name = "Total Crash points per SA2 (normalise)")
```
...
nb <- dnearneigh(coordinates(jtw_driving_akl_SA2_cas_dep_proj_sp_2),0,1000) 
...
mapview(sa2_clipped_proj_akl)

grid_spacing <- 1000 # 1000 meters


regional_sf <- st_read("../data/statsnzregional-council-2018-clipped-generalised-SHP/regional-council-2018-clipped-generalised.shp")

mapview(regional_sf)

mapview(regional_sf[regional_sf$REGC2018_1 == 'Auckland Region', ])

polygony <- st_make_grid(regional_sf[regional_sf$REGC2018_1 == 'Auckland Region', ], square = T, cellsize = c(grid_spacing, grid_spacing)) %>% # the grid, covering bounding box
  st_sf()


plot(polygony, col = 'white')
plot(st_geometry(regional_sf[regional_sf$REGC2018_1 == 'Auckland Region', ]), add = T)

# this is the correct way of doing it?
plot(st_geometry(regional_sf[regional_sf$REGC2018_1 == 'Auckland Region', ]), col = 'white')
plot(polygony[regional_sf[regional_sf$REGC2018_1 == 'Auckland Region', ]], col = '#ff000088', add = TRUE)


##################


polygony <- st_make_grid(sa2_clipped_proj_akl, square = T, cellsize = c(grid_spacing, grid_spacing)) %>% # the grid, covering bounding box
  st_sf() # not really required, but makes the grid nicer to work with later


plot(polygony, col = 'white')
plot(st_geometry(sa2_clipped_proj_akl), add = T)
...
```{r}
class(sa2_clipped_proj_akl)

## create a grid of size 500
g <- st_make_grid(sa2_clipped_proj_akl, cellsize = 500) ###
plot(g)
...
class(g[sa2_clipped_proj_akl])


SA2_akl_proj_grid <- g[sa2_clipped_proj_akl]

class(cas_data_akl_proj)
